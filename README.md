...
```bash
python3 train_lower.py /path/to/data --num-shots 1 --num-ways 20 --download --use-cuda
```
### Sharp-MAML (upper/both)
To run Sharp-MAML (upper) or Sharp-MAML (both) use: 
```bash
python3 train_both.py /path/to/data --num-shots 1 --num-ways 20 --download --use-cuda
```
